...
if uploaded_file:
    ...
    if search_type == "City":
        ...
        # Provide suggestions if typed >= 2 letters
    ...
    if search_btn:
        ...
        if search_type == "City":
            ...
            elif candidate_city not in CA_CITIES:
                st.error("City not found in California cities database.")
            else:
                candidate_coord = CA_CITIES[candidate_city]
        else:
            if not candidate_zip:
                st.error("Please enter a valid 5-digit California ZIP code.")
            elif candidate_zip not in ZIP_TO_COORD:
